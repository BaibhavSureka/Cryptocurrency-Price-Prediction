...
# Function to make predictions based on new input data
def predict_outcomes(days_since_high, diff_from_high, days_since_low, diff_from_low):
    ...
    # Predict outcomes
    high_pred = model_high.predict(X_new)[0]
    low_pred = model_low.predict(X_new)[0]
    
    return {'Predicted % Diff From High Next 5 Days': high_pred, 'Predicted % Diff From Low Next 5 Days': low_pred}
